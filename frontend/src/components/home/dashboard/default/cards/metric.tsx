...
export default function WeeklyStudyTimeCard({
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/dashboard/weekly-study?user_id=${user_id}`)
      .then((res) => res.json())
      .then((res) => {
        setData(res.data || []);
        setLastWeekTotal(res.last_week_total || 0);
      })
        ...
        <div className="mt-6 space-y-3 pt-4 border-t border-border">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2">
              <span
                className={`text-sm font-medium ${isIncrease ? "text-success" : "text-destructive"
                  }`}
              >
