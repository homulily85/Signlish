...
class SignLanguageProcessor:
    def __init__(self, model, encoder, threshold=1.2, complexity_setting=0):
        # --- Initialization ---
        self.model = model
    ...
    def extract_landmarks(self, results):
        ...
        pose = [(lm.x, lm.y) for i, lm in enumerate(results.pose_landmarks.landmark) if
                i in pose_indices_to_keep] if results.pose_landmarks else [(0, 0)] * len(
            pose_indices_to_keep)
        face = [(results.face_landmarks.landmark[i].x, results.face_landmarks.landmark[i].y) for i
                in
                important_face_landmarks] if results.face_landmarks else [(0, 0)] * len(
            important_face_landmarks)
        left_hand = [(lm.x, lm.y) for lm in
                     results.left_hand_landmarks.landmark] if results.left_hand_landmarks else [(0,
                                                                                                 0)] * 21
        right_hand = [(lm.x, lm.y) for lm in
                      results.right_hand_landmarks.landmark] if results.right_hand_landmarks else [(0,
                                                                                                    0)] * 21
    ...
    def preprocess(self, X):
        ...
        X[[2, 4, 6, 8, 10]] = preprocessing.AnchorNorm(X[[2, 4, 6, 8, 10]],
                                                       preprocessing.distance(X[0], X[2]), X[0])
        X[[3, 5, 7, 9, 11]] = preprocessing.AnchorNorm(X[[3, 5, 7, 9, 11]],
                                                       preprocessing.distance(X[1], X[3]), X[1])
    ...
    def process_and_annotate_frame(self, frame):
        ...
        with self.lock:
            ...
            cv2.putText(frame, status_text, (70, 50), cv2.FONT_HERSHEY_SIMPLEX, 1, status_color, 2,
                        cv2.LINE_AA)
            ...
            if self.interpreting:
                cv2.putText(frame, "Interpreting...", (frame.shape[1] - 220, 30),
                            cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                            (255, 0, 0), 1, cv2.LINE_AA)

        return frame

    def draw_last_predictions(self, frame, predictions, x=10, y=70, line_height=30, font_scale=0.7,
                              thickness=2):
        overlay = frame.copy()
        font = cv2.FONT_HERSHEY_SIMPLEX
        width = max(cv2.getTextSize(p, font, font_scale, thickness)[0][0] for p in
                    predictions) if predictions else 0
        ...
        for i, word in enumerate(predictions):
            cv2.putText(frame, word, (x, y + i * line_height), font, font_scale, (255, 255, 255),
                        thickness,
                        cv2.LINE_AA)

    def show_interpreted_text(self, frame, text):
        font, font_scale, thickness, color, bg_color = cv2.FONT_HERSHEY_SIMPLEX, 1, 3, (0, 255,
                                                                                        255), (50,
                                                                                               50,
                                                                                               50)
        ...
        cv2.rectangle(frame, (text_x - 20, text_y - text_h - 20),
                      (text_x + text_w + 20, text_y + 20), bg_color, -1)
        cv2.putText(frame, text, (text_x, text_y), font, font_scale, color, thickness, cv2.LINE_AA)

    def process_frame_get_results(self, frame):
        # 1. Standard Preprocessing (Keep this)
        frame = cv2.flip(frame, 1)
        image_rgb = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)
        results = self.holistic.process(image_rgb)

        # Note: We skipped the "draw_landmarks" part here!

        numpy_data = self.extract_landmarks(results)
        data_processed = self.preprocess(numpy_data)

        current_status = "Not Capturing"
        new_prediction = None
            ...
            if len(self.landmark_stored) > 5:
                ...
                if smoothed_score > self.threshold:
                    ...
                    if self.counter > 0:
                        ...
                        new_prediction = prediction_result

            # 3. Return Pure Data (No Images!)
            return {
                "status": current_status,
                "new_prediction": new_prediction,  # The word just detected (if any)
                "gloss_sequence": self.last_predictions,  # The full list of words
                "interpretation": self.current_interpretation,  # The full sentence
                "is_interpreting": self.interpreting
            }